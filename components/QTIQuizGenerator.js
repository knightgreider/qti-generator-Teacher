...
export default function QTIQuizGenerator() {
  ...
  const generateIMSCC = async () => {
    ...
    const qtiItems = questions.map((q,i) => {
      ...
      // metadata fields
      let metaFields = `          <qtimetadatafield><fieldlabel>cc_profile</fieldlabel><fieldentry>${isEssay ? 'cc.essay.v0p1' : (q.choices.length > 2 ? 'cc.multiple_choice.v0p1' : 'cc.true_false.v0p1')}</fieldentry></qtimetadatafield>`;
      if (isEssay) {
...
${choicesXml}
      ...
      // processing block
      const correctComparison = isEssay ? '' : `<varequal respident=\"response${idx}\">${q.answer+1}</varequal>`;
...
<manifest identifier=\"MANIFEST1\" xmlns=\"http://www.imsglobal.org/xsd/imscp_v1p1\" xmlns:imsqti=\"http://www.imsglobal.org/xsd/imsqti_v1p2\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.imsglobal.org/xsd/imscp_v1p1 imscp_v1p1.xsd\">
  ...
  return (
    <div style={{ padding: '1rem', fontFamily: 'sans-serif' }}>
      ...
      <textarea placeholder=\"Paste questions (Aiken, Gift, MC::, TF::, ES::, or ESSAY::)\" value={rawInput} onChange={e => setRawInput(e.target.value)} style={{ width: '100%', height: 150, marginBottom: 8 }} />
