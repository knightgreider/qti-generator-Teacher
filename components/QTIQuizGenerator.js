...
${questions.map((q,i) => {
      ...
      const profile = isEssay ? 'cc.essay.v0p1' : (q.choices.length > 2 ? 'cc.multiple_choice.v0p1' : 'cc.true_false.v0p1');
      return `      <item ident="${idx}" title="Question ${idx}">
        <itemmetadata><qtimetadata><qtimetadatafield><fieldlabel>cc_profile</fieldlabel><fieldentry>${profile}</fieldentry></qtimetadatafield></qtimetadata></itemmetadata>
        <presentation>
...
${q.choices.map((c,j) => `              <response_label ident="choice${j+1}"><material><mattext texttype="text/plain">${c}</mattext></material></response_label>`).join("\n")}
        ...
        <resprocessing>
          ...
          <respcondition continue="No">
            <conditionvar>${isEssay?'':`<varequal respident="response${idx}">${q.answer+1}</varequal>`}</conditionvar>
